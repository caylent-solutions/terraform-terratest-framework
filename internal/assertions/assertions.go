...
func AssertFileExists(t *testing.T, ctx interface {
	...
	// Clean the path - handle paths with ./ prefix
...
func AssertFileContent(t *testing.T, ctx interface {
	GetOutput(t testing.TB, key string) string
	...
	// If expected content is provided as parameter, use it
	if len(expectedContent) > 0 {
		contentToCheck = expectedContent[0]
	} else {
		// Otherwise get the expected content from the output
		contentToCheck = ctx.GetOutput(t, "output_content")
	}
	...
	if string(content) != contentToCheck {
		t.Fatalf("Expected file content to be %q, but got %q", contentToCheck, string(content))
	}
}

// AssertOutputEquals checks if a specified Terraform output matches an expected value
func AssertOutputEquals(t *testing.T, ctx interface {
	GetOutput(t testing.TB, key string) string
}, outputName string, expectedValue string) {
	t.Helper()
...
func AssertOutputContains(t *testing.T, ctx interface {
	GetOutput(t testing.TB, key string) string
}, outputName string, expectedSubstring string) {
	t.Helper()
...
func AssertOutputMatches(t *testing.T, ctx interface {
	GetOutput(t testing.TB, key string) string
}, outputName string, pattern string) {
	t.Helper()
...
func AssertOutputNotEmpty(t *testing.T, ctx interface {
	GetOutput(t testing.TB, key string) string
}, outputName string) {
	t.Helper()
...
func AssertOutputEmpty(t *testing.T, ctx interface {
	GetOutput(t testing.TB, key string) string
}, outputName string) {
	t.Helper()
...
func AssertOutputJSONContains(t *testing.T, ctx interface {
	GetOutput(t testing.TB, key string) string
}, outputName string, key string, expectedValue interface{}) {
	t.Helper()
