...
// and at any submodule depth (`module.example.module.inner.<type>...`).
// resourceType is matched as a complete address segment (it must be followed by
// a `.`), so counting "aws_lb" never matches "aws_lb_listener".
//
...
func matchModuleExampleResource(resourceType string) *regexp.Regexp {
	return regexp.MustCompile(fmt.Sprintf(`module\.example(\[[^\]]*\])?\.(.*\.)?%s\.`, regexp.QuoteMeta(resourceType)))
}
